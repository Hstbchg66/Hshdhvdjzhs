...
def create_code(message):
    ...
    if user_id == owner_id or user_id in admin_list:
        try:
            ...
            bot.reply_to(message, f"New code generated: <code>{code}</code> (valid for {hours} hours)")
        except:
            bot.reply_to(message, "Usage: /code [hours]")
    else:
        bot.reply_to(message, "❌ ليس لديك صلاحية لإنشاء أكواد VIP.")
...
def redeem_code(message):
    ...
    with open("codes.txt", "w") as file:
        for line in lines:
            ...
            if saved_code == code and float(expiration_time) > time.time():
                valid = True
                with open("premium.txt", "a") as premium_file:
                    premium_file.write(f"{user_id}:{expiration_time}\n")
                bot.reply_to(message, "Code redeemed successfully! You are now VIP.")
            else:
                file.write(line)
    if not valid:
...
# أمر /plan لإظهار تفاصيل الخطة للمستخدم
        ...
        if str(user_id) == saved_user_id:
            ...
            if remaining_time > 0:
                ...
                caption = (f"VIP Status:\n"
                           ...
                           "Thank you for being a VIP user!")
...
free_access_until = None  # Variable to store the free access expiration time

# Function to check if a user is registered
def is_user_registered(user_id):
    # Implement your registration check logic here
    # Example: checking in a file or database if the user is registered
    return True  # Modify this based on actual logic

# File to store the free access expiration time
FREE_ACCESS_FILE = "free.txt"

# Load the free access expiration time from file (if exists)
def load_free_access():
    if os.path.exists(FREE_ACCESS_FILE):
        with open(FREE_ACCESS_FILE, "r") as file:
            data = file.read().strip()
            if data:
                free_access_time = datetime.fromisoformat(data)
                # Check if the free period has expired
                if datetime.now() > free_access_time:
                    os.remove(FREE_ACCESS_FILE)  # Delete the file if the period has expired
                    return None
                return free_access_time
    return None

# Save the free access expiration time to file
def save_free_access(expiration_time):
    with open(FREE_ACCESS_FILE, "w") as file:
        file.write(expiration_time.isoformat())

# Delete the free access file
def delete_free_access_file():
    if os.path.exists(FREE_ACCESS_FILE):
        os.remove(FREE_ACCESS_FILE)

# Variable to store the free access expiration time
free_access_until = load_free_access()  # Load the time when the bot starts
...
# Command /free to allow free access to /chk for a certain number of hours
@bot.message_handler(commands=["free"])
def enable_free_access(message):
    global free_access_until

    try:
        # Extract the number of hours from the command
        args = message.text.split()
        if len(args) != 2:
            raise ValueError("Invalid arguments. Usage: /free <hours>")

        hours = int(args[1])  # Number of hours
        free_access_until = datetime.now() + timedelta(hours=hours)  # Set the expiration time

        # Save the free access expiration time to the file
        save_free_access(free_access_until)

        # Inform the admin that free access is enabled
        bot.reply_to(message, f"✅ Free access to /chk enabled for {hours} hours.")
    ...
    if message.chat.id == id:
        with open("combo.txt") as file:
            ...
            for cc in lino:
                ...
                progress_button = types.InlineKeyboardButton(f"🔄 Progress: {ok}/{total}", callback_data='progress')
...
# Add a handler for callback data for progress
@bot.callback_query_handler(func=lambda call: call.data == 'progress')
def callback_progress(call):
    bot.answer_callback_query(call.id, f"Progress: {ok}/{total}")

# Add a handler for callback data for time taken
...
def start(message):
    ...
    if free_access_until is not None and datetime.now() < free_access_until:
        ...
        if not 'pro' in found and user_id != owner_id:
            rank = "Free"
            keyboard = types.InlineKeyboardMarkup()
            contact_button = types.InlineKeyboardButton(text="- 𝗠𝗼𝗵𝗮𝗺𝗲𝗱 𝗛𝗮𝗺𝗱𝘆 .", url="https://t.me/Mohamed_Was_Here")
